...
// 퇴사까지 남은 N일을 7이라고 가정
// 걸리는 일 수와 수당금액을 counsel 데이터로 가정

let N = 7
var counsel = [[3, 10], [5, 20], [1, 10], [1, 20], [2, 15], [4, 40], [2, 200]]
var max = 0

func dfs(begin: Int, profit: Int) {
    
    let end = begin + counsel[begin][0]
    ...
    for next in end..<N {
        dfs(begin: next, profit: profit + counsel[next][1])
    }
    
}

for i in 0..<N {
    dfs(begin: i, profit: counsel[i][1])
}
